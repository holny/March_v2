...
public class UserEmailRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        // 以下是自定义的根据用户数据库中权限信息对当前账户授予相应shiro权限。
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException  {
        ...
        List<User> users = userService.findUserByEmail(userAccount);
        ...
        if(UserController.AccountStatusEnum.FORBIDDEN_STATUS.getCode().equals(user.getAccountStatus())) throw new DisabledAccountException();       // 此账户状态有异常
        if(UserController.AccountStatusEnum.DELETED_STATUS.getCode().equals(user.getAccountStatus())) throw new LockedAccountException();       // 此账户状态有异常

        // 交给AuthenticationRealm使用CredentialsMatcher进行密码匹配。
        SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(
                user, // 用户名,经试验，这个不只对密码验证有关系(必须有密码字段)，还会往下传递给doGetAuthorizationInfo()方法，所以我这里直接传User对象就不用再查数据库了
